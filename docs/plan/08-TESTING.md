...
The integration PR filter also includes root `pubspec.yaml` and
`pubspec.lock`: `dart pub get` resolves the workspace there, so dependency
changes must exercise SSH even without package source edits.
These inputs use the same fixture-presence guard as package source changes.
