...
- **Explicit test/analyze paths, always.** A bare `dart test` or
  ...
  dart analyze packages/poltergeist_sync   # explicit roots (08 §1)
...
void validateRelativeComponents(String relative,
  ...
  for (final part in relative.split('/')) {
    ...
    if (part.isEmpty || part == '.' || part == '..' || part.contains('\\') ||
        ...
        part.contains('\n') || part.contains('\r') ||
        // U+2028/U+2029 are line/paragraph separators in Unicode and to
        // most line-oriented tooling — same escaping-bug class as '\n'.
...
// own name (crash sweeps must additionally require the middle
// segment to parse as a UUID — a bare `.poltergeist-*.tmp` glob
// would also sweep a user file legitimately so named).
final tmp = File(
...
- A **stalled** PR (neither merged, closed, nor at steady-state, with the
  ...
  activity (activity generated by the agent's own check-ins or triage
  comments does not count — only human/CI activity restarts the clock),
  run the full teardown — `unsubscribe_pr_activity` **and**
