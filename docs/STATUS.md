...
M2 has started: pooled connection growth and extra-transport idle teardown
are in (open item 4 tracks the remaining M2 slices)._
...
| M2 — extra-transport idle teardown | Extra transports close after 60 seconds without channels or pending channel opens/closes. Returned transfer channels serve waiters first, then close on extras so caches cannot prevent retirement (03 §3.3 clarification). The first transport keeps its cache and follows pane/lease lifetime. Eighteen fake-clock tests cover deadlines, renewed demand, shared bookmarks, queued handoff, delayed cleanup, teardown races, and waking acquisitions queued during channel retirement. |
