...
| M2 — extra-transport idle teardown | Extra transports close after 60 seconds without channels or pending channel opens/closes. Returned transfer channels serve waiters first, then close on extras so caches cannot prevent retirement (03 §3.3 clarification). The first transport keeps its cache and follows pane/lease lifetime. Fake-clock tests cover deadlines, renewed demand, shared bookmarks, queued handoff, delayed cleanup, and teardown races. |
