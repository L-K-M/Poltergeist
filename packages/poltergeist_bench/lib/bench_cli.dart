...
/// Bench CLI body, exposed as a library entrypoint so the relocated package
/// and the legacy `tool/bench` compatibility entrypoint run identical code.
Future<void> benchMain(List<String> arguments) async {
  ...
  if (parsed['help'] as bool || parsed.rest.length != 1) {
    stdout.writeln('Usage: dart run benchmark/bench.dart <scenario> [options]');
    stdout.writeln(
