// Ported from Séance app/seance_app/lib/ui/server_appearance.dart @ ded9228; see docs/PORTS.md.
...
/// list, so the twenty possible results are computed once and kept. The
...
/// A human name for [icon], for the picker's labels and tooltips — and the
/// badge's semantics fallback, where an untagged row still announces
/// something. Upstream vocabulary, intentionally not ARB: these are glyph
/// names, not product copy.
...
/// A human name for [color], for the editor's swatch tooltips. Same
/// upstream-vocabulary posture as [serverIconLabel].
...
///
/// The sidebar draws its copy of the line through the shared kit's
/// `SidebarRow.accent` (D33), in both densities; this widget is the
/// editor's and the colour picker's preview of it, so [width] and the
/// kit's line stay the same 4 px.
...
class ServerBadge extends StatelessWidget {
  ...
  /// The corner radius, as a share of the side. Kept as the one radius source
  /// so every badge-bearing surface draws the same shape.
