// Ported from Séance app/seance_app/windows/runner/settings_window.cpp @ 38b7a42; see docs/PORTS.md.
...
constexpr wchar_t kWindowTitle[] = L"Poltergeist Settings";
...
class SettingsFlutterWindow : public Win32Window {
 ...
 protected:
  ...
  LRESULT MessageHandler(HWND hwnd, UINT const message, WPARAM const wparam,
    ...
    // down is not safe (lib/services/settings_window/settings_window_host.dart
    // says why). It also reopens at once; the Dart side drops its screen
    // while hidden.
    if (message == WM_CLOSE) {
