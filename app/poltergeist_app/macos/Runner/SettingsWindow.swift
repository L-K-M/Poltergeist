// Ported from Séance app/seance_app/macos/Runner/SettingsWindow.swift @ 38b7a42; see docs/PORTS.md.
import Cocoa
import FlutterMacOS

/// The Settings window: a second NSWindow on a second Flutter engine, whose
/// Dart side runs the Settings sections instead of the app (see
/// lib/services/settings_window/settings_window_host.dart).
///
/// Dart opens it, or brings it forward, with "open" on the app engine's
/// `poltergeist/settings_window` channel, and hears "closed" there when the
/// user closes it. Every message either engine sends on
/// `poltergeist/settings_link` is forwarded to the other engine, and its
/// reply back: the two engines share nothing else, so this is how the window
...
/// second engine down is not safe (settings_window_host.dart says why). It
/// also reopens at once; the Dart side drops its screen while hidden.
final class SettingsWindowHost: NSObject, NSWindowDelegate {
  private static let controlChannel = "poltergeist/settings_window"
  private static let linkChannel = "poltergeist/settings_link"
  ...
  private static let windowArgument = "--poltergeist-settings-window"
  ...
  private func open() {
    ...
    let controller = PoltergeistFlutterViewController(project: project)
