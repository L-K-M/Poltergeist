// Ported from Séance app/seance_app/linux/runner/settings_window.cc @ 38b7a42; see docs/PORTS.md.
...
constexpr char kControlChannel[] = "poltergeist/settings_window";
constexpr char kLinkChannel[] = "poltergeist/settings_link";
...
constexpr char kWindowArgument[] = "--poltergeist-settings-window";

constexpr char kWindowTitle[] = "Poltergeist Settings";
...
void open_window(SettingsWindowHost* host) {
  ...
  // Transparent, like the app's window, so the GTK background (which
  // follows the desktop's light/dark theme) shows through during a resize.
...
void settings_window_install(GtkApplication* application,
  ...
  g_object_set_data_full(G_OBJECT(main_window), "poltergeist-settings-window",
                         host, host_free);
}
