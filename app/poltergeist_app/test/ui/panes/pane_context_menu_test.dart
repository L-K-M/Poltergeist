...
void main() {
  ...
  Future<void> pumpPane(
    ...
    late final List<RegisteredCommand> commands;
    commands = [
      ...buildPaneCommands(
        workspace: workspace,
        focusLeft: () {},
  ...
  testWidgets('the empty area opens the folder-level menu', (tester) async {
    await openRows();
    await pumpPane(tester);
    ...
    expect(item(kFileNewFolderCommandId), findsOneWidget);
  ...
  testWidgets('Shift+F10 opens the menu over the cursor row with focus on '
    ...
    final firstFocus = Focus.of(
      tester.element(
        find
            .descendant(of: item(kGoOpenCommandId), matching: find.byType(Text))
            .first,
      ),
    );
