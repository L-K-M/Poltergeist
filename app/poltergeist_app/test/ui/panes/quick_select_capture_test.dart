...
/// Real-font captures of the Quick Select strip (02 §2.5) for visual
...
void main() {
  testWidgets('captures the Quick Select strip in both modes and closed', (
    tester,
  ) async {
    ...
    final field = find.byKey(
      const ValueKey('pane.left.quickSelect.field'),
    );
    int rowOf(String name) =>
        left.entries.indexWhere((e) => e.name == name);

    // Closed baseline.
    await capture('quick-select-closed');

    // A mixed opening selection so both modes read clearly: the toggle
    // must recompute from it, not from the last preview.
    left.setCursorIndex(rowOf('docs'));
    left.setCursorIndex(
      rowOf('photo-01.png'),
      update: SelectionUpdate.toggle,
    );

    // Open, Add mode, live fragment preview: docs + photo-01 stay, the
    // three .txt rows join.
    left.openQuickSelect();
    await tester.pump();
    await tester.enterText(field, '.txt');
    await tester.pump();
    expect(left.selectedCount, 5);
    await capture('quick-select-open-add');

    // Remove mode over the same query drops the matched .txt rows and
    // keeps the manually selected baseline rows.
    ...
    expect(left.isRowSelected(rowOf('photo-01.png')), isTrue);
    await capture('quick-select-open-remove');

    // Esc closes and restores.
    await tester.sendKeyEvent(LogicalKeyboardKey.escape);
    await tester.pump();
    expect(left.quickSelectActive, isFalse);
    await capture('quick-select-after-esc');
  });
}
