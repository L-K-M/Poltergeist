...
**[docs/plan/](docs/plan/)** — read its overview (the decision log) before
writing app code, then follow [docs/plan/09-PLAYBOOK.md](docs/plan/09-PLAYBOOK.md).
...
- Analyze/test the pure-Dart packages with explicit paths, never bare at the
  repo root (once the Flutter app exists, a bare `dart test` would try to
  build it and fail): `dart test packages/poltergeist_core`.
- The app will live at `app/poltergeist_app` (Flutter); its platform folders
  get committed once scaffolded (they carry the app name, icons, and macOS
  entitlements). Keep the product name ASCII in bundle file names — codesign
  rejects accented file names (learned in Séance).
- Sibling repo [Séance](https://github.com/L-K-M/Seance) is the reference
  implementation for SSH/SFTP transport, TOFU, vault, sync, and the editor —
  read its AGENTS.md before porting code either way.
